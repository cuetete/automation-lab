...
// InitPeripherals(); // Not required for this example
   ...
   //CCW
   j=512;   //Modify this parameter to change the steps of stepper motor
   while(j--)
   {
      for(i=0;i<4;i++)
      {
	 ...
	 while(j--)
   {
      for(i=0;i<4;i++)
      {
       GpioDataRegs.GPADAT.all=Step_table1[i]; //set GPIO2-GPIO5 High one by one in the opposite direction
       delay_loop(80000);
